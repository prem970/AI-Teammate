  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-[#040711] relative overflow-hidden">
      ...
      <header className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex items-center justify-between">
        ...
        <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-surface-secondary border border-surface-border text-xs font-mono text-slate-400">
          <span className="w-2 h-2 rounded-full bg-slate-400" />
          <span>Demo admin login · sample topology pages</span>
        </div>
      </header>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 my-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        <div className="lg:col-span-7 space-y-6">
          ...
          <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-white leading-[1.12]">
            ...
            <span className="text-gradient-cyan">architecture</span> catalog.
          </h1>

          <p className="text-slate-300 text-base sm:text-lg max-w-2xl leading-relaxed font-sans">
            Integrations status is derived from env. Agent topology and traces are documentation
            samples unless wired to audit stores.
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 text-xs font-mono text-slate-300">
            <div className="p-3 rounded-xl bg-surface-secondary/60 border border-surface-border">
              ...
              <p className="font-bold text-white mb-0.5">Agent catalog</p>
              <p className="text-[11px] text-slate-400">SSOT architecture — not live heartbeats.</p>
            </div>
            <div className="p-3 rounded-xl bg-surface-secondary/60 border border-surface-border">
              <ShieldCheck className="w-4 h-4 text-emerald-400 mb-1.5" />
              <p className="font-bold text-white mb-0.5">MCP probe</p>
              <p className="text-[11px] text-slate-400">Reachability check against MCP_BASE_URL.</p>
            </div>
            <div className="p-3 rounded-xl bg-surface-secondary/60 border border-surface-border">
              <Terminal className="w-4 h-4 text-purple-400 mb-1.5" />
              <p className="font-bold text-white mb-0.5">Sample traces</p>
              <p className="text-[11px] text-slate-400">Illustrative deny/allow examples.</p>
            </div>
          </div>
        </div>
          ...
          <div className="p-7 sm:p-8 rounded-2xl bg-[#090d1a]/95 border border-surface-border hover:border-cyan-500/40 shadow-2xl backdrop-blur-xl transition-all">
            <div className="mb-6">
              ...
              <p className="text-xs text-slate-400 mt-1">
                Demo cookie auth — any password accepted for the preset admin email.
              </p>
            </div>
            ...
            <div className="mb-5 p-3 rounded-xl bg-surface-secondary border border-surface-border flex items-center justify-between">
              <div>
                ...
                <span className="block text-xs font-bold text-white">Platform Super Admin</span>
                <span className="block text-[10px] font-mono text-cyan-400">
                  admin.super@company.mock
                </span>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2.5 rounded-lg bg-surface-secondary border border-surface-border text-sm text-white focus:outline-none focus:border-cyan-400"
                  />
                </div>
              </div>
      ...
      <footer className="relative z-10 border-t border-surface-border/40 py-4 px-4 text-center text-xs text-slate-500 font-mono">
        Autonomous AI OS © 2026 • Platform Admin Console
      </footer>
    </div>
  );
}
